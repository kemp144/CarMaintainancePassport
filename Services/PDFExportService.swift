...
struct PDFExportService {
    ...
    private func renderPassportCover(in ctx: CGContext, bounds: CGRect, vehicle: Vehicle) {
        ...
        if serviceCount == 0 {
            ...
            summaryText = "This document contains the verified service history for this vehicle as recorded in the Car Maintenance Passport app. It includes \(serviceCount) service \(serviceCount == 1 ? "entry" : "entries") totalling \(AppFormatters.currency(vehicle.totalSpent, code: vehicle.currencyCode)), with the most recent service on \(lastDateStr)."
        }
        ...
        ctx.strokePath()
        ctx.restoreGState()
        draw(text: "Car Maintenance Passport  ·  \(vehicle.title)  ·  Page 1",
    ...
    private func renderPassportServicePage(in ctx: CGContext, bounds: CGRect,
        ...
        ctx.strokePath()
        ctx.restoreGState()
        draw(text: "Car Maintenance Passport  ·  \(vehicle.title)  ·  Page \(pageNumber)",
             in: CGRect(x: 32, y: footerY, width: bounds.width - 64, height: 14),
             font: .systemFont(ofSize: 9), color: slate400.withAlphaComponent(0.6))
    ...
    private func renderResaleCover(in ctx: CGContext, bounds: CGRect,
        ...
        if services.isEmpty {
            bullets = [
                "✓  Service history available in Car Maintenance Passport",
                "✓  All future records will be documented and verifiable",
            ]
        } else {
            bullets = [
                "✓  Full service history documented in Car Maintenance Passport",
        ...
        ctx.addLine(to: CGPoint(x: bounds.width - 32, y: footerY - 8))
        ctx.strokePath()
        draw(text: "Generated by Car Maintenance Passport on \(AppFormatters.mediumDate.string(from: .now))  ·  All data entered by vehicle owner",
             in: CGRect(x: 32, y: footerY, width: bounds.width - 64, height: 14),
